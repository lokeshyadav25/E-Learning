...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Check file type and size if you want (example: only PDFs allowed)
    $allowedTypes = ['application/pdf', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'];
    if (!in_array($file['type'], $allowedTypes)) {
        die('Only PDF and Word documents are allowed.');
    }

    // Create directory for assignment uploads if not exists
    ...
    $filename = $student_id . '_' . $course_id . '_' . time() . '_' . basename($file['name']);
    $targetPath = $uploadDir . $filename;

    if (move_uploaded_file($file['tmp_name'], $targetPath)) {
        // Save assignment info in DB (optional)
